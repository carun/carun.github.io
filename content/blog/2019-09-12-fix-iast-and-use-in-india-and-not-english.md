+++
title = "Roman Script for Indian Words"
date = 2019-09-12
+++
...
Over time, various Indian scripts—chief among them the [Brahmic scripts](https://en.wikipedia.org/wiki/Brahmic_scripts)—evolved to capture these sounds. Today, [Devanagari](https://en.wikipedia.org/wiki/Devanagari) is among the most widely used.

To bridge Indian languages and global readership, Roman transliteration became necessary. The [IAST system](https://en.wikipedia.org/wiki/International_Alphabet_of_Sanskrit_Transliteration), formalized in 1894, was created by Western scholars to provide a faithful, reversible mapping of Indian scripts using Roman characters. It was designed with scholarly precision, enabling readers to reproduce original sounds accurately.

## ISO 15919

Tamil is not romanized using IAST, but using [ISO 15919](https://www.iso.org/standard/28333.html). So there's a hegemony of their own arbitrary directions.
...
- The Tamil sound `த` is romaized as `t` but many read it as `ட`.
- The Hindi `दी` in *Modi* becomes `Mōdī` in IAST and `Modi` in English—but Tamil speakers often read it as `Mōḍi`.
...
- Swap `T` and `Ṭ`: `T` should represent ट, and `Ṭ` should represent त.
- Swap `D` and `Ḍ`: `D` should represent ड, and `Ḍ` should represent द.
- Use `e` for short *e* (as in *egg*), and `ē` or `ee` for long *e* (as in *ace*).
- Use `o` for short *o* (as in *pot*), and `ō` or `oo` for long *o* (as in *door*).
- Always use lowercase letters when writing Indian languages in Roman script, to remove ambiguity between aspirated/unaspirated and voiced/unvoiced pairs.
- All the hard consonants in Tamil க,ச,ட,த,ப must use their phonetic equivalents when they come in the middle of the words instead of blindly using the primary equivalents. This is because unlike other indian langauges Tamil uses a single character to represent multiple sounds. In that sense, it is phonetically inconsistent.
    * For instance, Tamil words written in IAST are using `Ṭ` when `ட` comes in the middle - this is wrong. Eg: கொடுத்தான் is currently written as koṭuttāṉ. It should've been written as koḍutthān. Even that must be changed as per earlier suggestion to be written as koduṭṭāṉ.
